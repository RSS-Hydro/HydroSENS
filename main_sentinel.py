...
# Load and select Sentinel-2 image
...
date = str(date)
print(date)
resample = resampling(mosaic, shapefile_projection)
DEM = getDEM(aoi)
Bandsexport(resample, shapefile_projection, output, aoi)
...
output_raster = driver.Create(output + r"\bands_masked.tif", bands.RasterXSize, bands.RasterYSize, band_array.shape[0],
                              gdal.GDT_Float64)
